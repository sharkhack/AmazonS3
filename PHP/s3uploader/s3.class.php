 ...
 *
 *
 *  UPDATE S3.CLASS.CONF.PHP GLOBAL VARIABLES
 *
 */
	...
	*/
	function s3($options = NULL) {
		...
		if($this->bucketname == NULL)
			$this->bucketname = $GLOBALS['bucketname'];
		...
		$this->hasher =& new Crypt_HMAC($GLOBALS['secretKey'], "sha1");
	...
	function sendRequest ($resource, $verb = NULL, $objectdata = NULL, $acl = NULL, $contentType = NULL, $metadata = NULL)
	{
		...
		if (is_array($metadata)) {
			...
			foreach ($metadata as $key => $value) {
				if($key!='Cache-Control' && $key !='Expires'){
		...
		$req->addHeader("Authorization", "AWS " . $GLOBALS['keyId'] . ":" . $signature);
